		...
		switch(robot_state){
			...
			//rotate until target seen. If no target with 360 roation then move across arena more.
				...
				if ( beacon_level > beacon_threshold ) {	//light seen
					...
					wait1Msec(500);
					turnLeft(500);	//either turn past beacon then turn back in the next code or hard turn to fix rough IR detection
...
//actual program//
				if(SensorValue(ultrasonic) > 23){
					driveForwards(2);
				}
				//if too close to target
				//back led on
				else if(SensorValue(ultrasonic) < 21 && SensorValue(ultrasonic) > 0){
					driveBackwards(5);
				}
				//if reached target
				else{
//
					...
					if(SensorValue(ultrasonic) < 21 || SensorValue(ultrasonic) > 23) {
						break;
					}
