...
int DEGREES90 = 500; //amount of time to turn 90 degrees set to 3600?
int rotationSpeed;
...
void fullExecution(){
	...
	RobotState robot_state = SEARCHING; //for switch case
	...
	while(!jobDone){
		...
		switch(robot_state){
			...
			//rotate until target seen. If no target with 360 roation then move across arena more.
				//rotate
				turnLeft(20);
				amountTurned += 5;//degrees turned in one turnLeft(20)
			...
			case(ADJUST_SEARCH_POSITION):
				...
				for(int k = 0; k < 100; k++){
					...
					driveForwards(20);
					//detect walls
					if(SensorValue(ultrasonic) < 13){	//might need adjusting------------------------------
						driveBackwards(100);
						turnLeft(DEGREES90);
					}
					SensorValue[led1] = 1;
			...
			case(APPROACHING):
			//approach while checking for target lost
				//if lost beacon
				beacon_level = monitorLight();	//ir sensor difference update
				...
				//back led on
				else if(SensorValue(ultrasonic) < 13){
					SensorValue[led2] = 0;
					driveBackwards(5);
				}
				//if reached target
