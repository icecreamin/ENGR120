...
*/
			...
			case(BEST_EXIT): //get away from target without being connected or tangled with the cable
				//raise arm here to avoid reading the arm as a distance. Make sure arm doesnt hit the floor----------------------------------------------
				raiseArm(300);
				tabby1_pushed = false;
				
				//turn left 90, keep turning until 60 cm clearence then drive away
				turnLeft(DEGREES90);
				...
				while(!exit_found){
					...
					else if( SensorValue[ultrasonic] > 100 ){
						exit_found = true;
					}
				}
			...
			//drive away. Don't hit walls
				...
				//if rams a wall then stop
				monitorInput();
				if(tabby1_pushed || tabby2_pushed){
					case = FINISHED;
				}
				
				//if run into wall or moved for 200 cycles stop
				if(SensorValue[ultrasonic] < 30 || exitDistance > 200){					//ADD TABBY HIT STOPS ROBOT
